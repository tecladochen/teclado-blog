...
**l2**：测量向量之间的绝对几何距离，适用于想要真正的空间接近。

$$
d = \sum_{i=1}^{n} (A_i - B_i)^2
$$

**ip**：关注向量对齐和大小，通常用于推荐系统。

$$
d = 1.0-\sum_{i=1}^{n} A_i B_i
$$

**cosine**：仅测量矢量之间的角度（忽略大小），非常适合文本嵌入的情况。

$$
d =1.0- \frac{\sum_{i=1}^{n} A_i B_i}{\sqrt{\sum_{i=1}^{n} A_i^2} \sqrt{\sum_{i=1}^{n} B_i^2}}
$$
